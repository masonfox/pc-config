...
Execute up to **10 iterations**. Each iteration:
...
### Step 3: Get GitHub Copilot Feedback

1. **Request Copilot analysis**:
   ```bash
   gh pr edit --add-reviewer @copilot
   ```

2. **Wait for response**: Sleep for 5 minutes to allow Copilot to respond
   ```bash
   sleep 300
   ```

3. **Extract Copilot's response**:
   ```bash
   # Get latest Copilot comment
   gh api repos/{owner}/{repo}/issues/<number>/comments --jq 'map(select(.user.login == "copilot-pull-request-reviewer[bot]")) | sort_by(.created_at) | last | .body'
   ```

   Be aware that the copilot reviewer name can differ from just "copilot", such as "copilot-pull-request-reviewer[bot]"

4. **Poll if needed**: If no response after initial wait, poll up to 20 more times (30 sec each):
   - Wait 30 seconds between polls
   - Check for new Copilot comment

**Parse Copilot's response**:
- If no response after timeout → `copilot_approved = true` (continue anyway)
- If response contains "LGTM" or "looks good" or "no issues" → `copilot_approved = true`
- Otherwise → `copilot_approved = false`, extract suggestions

**Progress indicators**:
- Announce: "⏳ Requesting GitHub Copilot feedback via comment..."
- After initial request: "⏳ Waiting for Copilot response..."
- During polling: "🔍 Polling for Copilot response... (attempt N/20)"
- On timeout: "⚠️  Copilot didn't respond - continuing without feedback"
